...
public class JdbcTemplate {
	...
	public <T> T excuteQueryForObject(String sql, ResultMap<T> resultMap, String... inputParameters) {
		log.debug("JdbcTemplate excuteQuery sql : {}", sql);

		String replacedSql = setSqlParameter(sql, inputParameters);
	...
	public <T> List<T> excuteQueryForList(String sql, ResultMap<T> resultMap, String... inputParameters) {
		log.debug("JdbcTemplate excuteQuery sql : ", sql);
		...
		try {
			for (Method method : modelClass.getMethods()) {
				...
				if (getterMethodName.equals(method.getName())) {
					getterMethodValue = (String) method.invoke(model);
					break;
				}
			}
